...
export function resolveRequestToolIdentity(identityMap: unknown, toolName: string) {
  ...
  // 5. Check exact or case-insensitive bare leaf name
  for (const identity of candidates) {
    if (toolName === identity.name || toolLower === identity.name.toLowerCase()) {
      return identity;
    }
  }

  // 6. Check suffix match (e.g. tool_<hash>_exec or prefix__exec)
  for (const identity of candidates) {
