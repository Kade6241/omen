...
import { isModelLocked as defaultIsModelLocked } from "@omniroute/open-sse/services/accountFallback.ts";
...
async function isModelLockedOnAllConnections(
  providerAlias: string,
  modelId: string,
  deps: VisionBridgeRouterDeps
): Promise<boolean> {
  const checkLocked = deps.isModelLocked ?? defaultIsModelLocked;
  try {
    const { getProviderConnections } = await import("@/lib/db/providers");
    const connections = await getProviderConnections({ provider: providerAlias, isActive: true });
    if (!Array.isArray(connections) || connections.length === 0) return false;
    // If at least one connection is NOT locked for this model, the provider/model is still routable
    const fullModel = `${providerAlias}/${modelId}`;
    for (const c of connections as Array<{ id: string }>) {
      try {
        if (
          !checkLocked(providerAlias, (c as any).id, fullModel) &&
          !checkLocked(providerAlias, (c as any).id, modelId)
        ) {
          return false;
        }
      } catch {
    ...
    return true; // all connections locked for this model
  } catch {
    return false;
  }
}
...
async function getVisionCapableModels(
  ...
  const candidatesByProvider = await Promise.all(
    Object.entries(PROVIDER_MODELS).map(async ([providerAlias, models]) => {
      ...
      // Penalize models whose every active connection is model-locked (e.g. 402 quota on all 6 kilocode accounts)
      // Without this, getBestVisionModel keeps re-picking the same failing model for 60s via selectionCache
      if (usableModels.length > 0) {
        const lockedResults = await Promise.all(
          usableModels.map(async (m) =>
            (await isModelLockedOnAllConnections(providerAlias, m.id, deps)) ? null : m
          )
        );
        usableModels = lockedResults.filter((m): m is (typeof visionModels)[number] => m !== null);
      }
...
async function resolveCachedSelection(
  ...
  if (await cachedModelRemainsAvailable(cached.modelId, deps)) {
    // Invalidate cache if the cached model is now locked on all connections (402 on every kilocode account)
    const sep = cached.modelId.indexOf("/");
    if (sep > 0) {
      const cachedProvider = cached.modelId.slice(0, sep);
      const cachedModelId = cached.modelId.slice(sep + 1);
      if (await isModelLockedOnAllConnections(cachedProvider, cachedModelId, deps)) {
