...
export function shouldSelfProbeExit(consecutiveFailures, threshold) {
  return Number.isFinite(consecutiveFailures) && consecutiveFailures >= threshold && threshold > 0;
}
  ...
  const timer = setInterval(async () => {
    ...
    if (ok) {
      if (consecutiveFailures > 0) {
        log.warn?.(`[SelfProbe] recovered after ${consecutiveFailures} failed probe(s)`);
      }
